...
#%%
@pf.register_dataframe_method
def translate_text(df_col, lang="en"):
        '''
        ...
        df[f'text_{lang}']= [translator.translate(x) for x in (df_col)]
        return df

#%%
@pf.register_dataframe_method
def clean_text(df_col, **preprocess_kwargs):
    '''
    preprocess for text
    kwargs:
...
def pre_process(text, stop_words=stopwords, remove_tags=True, remove_special_characters=True, 
