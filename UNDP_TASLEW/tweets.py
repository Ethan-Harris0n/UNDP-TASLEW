 
'''Much of the source code or entire functions below were acquired from the following repositories:
            - https://github.com/dmbeskow/.
'''
#%%
def check_tweet(tweet):
    ...
    """
    import CREW_UNDP_Twitter
    if 'status' not in tweet.keys():
        if 'friends_count' in tweet.keys():
            tweet['status'] = CREW_UNDP_Twitter.get_empty_status()
...
def remove_bad_json_data(files):
    ...
    Example: CREW_UNDP_Twitter.remove_bad_json_data(files)
    '''
    import io, gzip, json
    import progressbar
...
def filter_tweets_by_date(files, start , stop, file_name):
    ...
    Example: CREW_UNDP_Twitter.filter_tweets_by_date(files, '2018-01-01', '2018-06-01','outfile.json')
    '''
    import io, gzip, json
    import progressbar
